...
1. **Wire and calibrate.** Servo signals straight to the Pi's hardware PWM pins, pan on GPIO18 (pin 12) and tilt on GPIO19 (pin 35), enabled by `setup-system.sh --servo-pwm` (`dtoverlay=pwm-2chan`; turns off the 3.5 mm audio jack, which shares that hardware) — or a PCA9685 on I2C, pan on channel 0, tilt on channel 1 — with a separate 5–6 V supply (buck converter) and a common ground. Never software PWM on an ordinary GPIO pin. The robot's servos are **continuous-rotation (360°) MG995s**, kept without a position sensor by the owner's decision: calibration measures each one's neutral pulse, stop band and speed, the head angle is dead-reckoned from them and drifts, and neutral (stop) is the safe state. Re-check the Phase 4 gaze and Phase 9 idle-motion acceptance criteria against that drift when those land. Write a standalone calibration script sweeping pulse widths, recording mechanical min/max, center, and pulse-to-angle mapping per servo. Store in `config/motion.local.yaml` (the servos are TowerPro MG995s; `config/motion.yaml` holds the committed profile and holds uncalibrated axes to a safe 1200–1800 µs window); put wiring and numbers in `docs/hardware.md`. `neo-servo-check --pulse CHANNEL US` drives one servo for the sweep.
2. **`servo_driver` node.** Subscribes `/head/command`, publishes `/head/state` at 50 Hz. In order:
   ...
   - deadband to stop buzzing on micro-corrections — but keep it small, because it also quantizes slow motion into steps of roughly its own size, and idle drift and distant tracking are both slow motion. Measured: a 2° idle sine renders as 58 jumps of 0.43° over 30 s at the 0.4° placeholder, versus 256 jumps of 0.12° at 0.1°. The PCA9685 at 50 Hz resolves only ~0.44° per step (4.9 µs of a 2000 µs range), so the servo itself cannot follow anything finer; the robot's MG995s are specified for 50 Hz, so a higher PWM frequency is not a way under that floor,
